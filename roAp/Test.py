...
mmag_lps = 0.0010857362047581294 * 1e6 * pg_low.power.value  # Convert to mmag for lower frequency Periodogram
mmag_ps = 0.0010857362047581294 * 1e6 * pg.power.value  # Convert to mmag for main Periodogram
...
for i, (f, h) in enumerate(zip(freq, peaks)):
    if h == max(mmag_ps):  # Central peak or very far from peak
        ax3.vlines(x=f, ymin=h, ymax=ymax*1.1, linestyle=':', color='red', label="Central Peak", zorder=0)  # Central
    else:
        ax3.vlines(x=f, ymin=h, ymax=ymax*1.1, linestyle='--', color='lightgrey', linewidth=0.8, zorder=0)   # Side lobes
...
ax3.axhline(y=threshold, color='blue', linestyle='--', label=f"Threshold ({num}"+r"$\sigma$)")  # Threshold line
...
ax4.set_title(f"Phase-folded Light Curve (Period = {(1/pmax):.3f} d)", fontweight='bold')
