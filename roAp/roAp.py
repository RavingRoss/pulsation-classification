...
class TimeDataTESS:
    ...
    def getShortestCadence(self, ra, dec):
        ...
        search = lk.search_lightcurve(
            f"TIC {tic_id}", author="SPOC", cadence="short"
        )  # author='SPOC', mission='TESS', exptime=120)
    ...
    def getTESSData(self, targetID):
        ...
        for row in cands.itertuples(index=False):
            ...
            print(stars)
            stars.to_csv(
                f"Data/TESS Data/Clusters/TESS Cluster {stars['Cluster Name'][0]}.csv"
            )
            return stars, lc, pg
    ...
    def plotTESSData(self, targetID):
        ...
        for i in range(len(stars)):
            ...
            isocmd = rmm.ISOCMD(
                "Data/MIST_iso_reddened.iso.cmd"
            )  # Importing ISOCMD image for isochrones
            ...
            # Plotting the CMD
            sc = ax1.scatter(
                bp_rp0, gmag0, c=prob0, cmap="viridis", label=f"{name}", s=10, zorder=1
            )
            ax1.scatter(bp_rp, gmag, c=prob, cmap="viridis", s=80, zorder=3, marker="*")
            ax1.plot(
                BP - RP, G + 10, label=f"Isochrone Age: {age:.3f}", c="m", zorder=1
            )
            ...
            plt.savefig(
                f"Data/TESS Data/Graphs/Final Plot of {id}",
                dpi=300,
                bbox_inches="tight",
            )
            # plt.show()
...
class Main:
    ...
    def plot_skew(self, path="Data/TESS Data/Skewed Density Plot"):
        ...
        xy = np.vstack(
            [np.concatenate([pgskewed, lcskewed]), np.concatenate([power, power])]
        )
        ...
        fig.suptitle(
            "Power vs. Skew Density", fontweight="bold", fontsize=14, x=0.44, y=0.95
        )
...
if __name__ == "__main__":
    ...
    stop_clust = len(
        clusters
    )  # will take a while, like 2 days for 'run' function, unless you change input value for Vizier query.
